...
int quad_roots(double *a,double *rr){
	...
	int l;
    l=lin_root(a,r);
	if (fabs(a[2])<1e-14) {lin_root(a,r); 
	...
	else if (fabs(a[2])>1e-14 && fabs(a[0])<1e-14) {lin_root(a,r); 
	                               if (r[0]>zero) {rr[1]=r[0]; rr[2]=zero;}
			                       else {rr[1]=zero; rr[2]=r[0];}
	                               return 2;}//r= real and 0, ordered
	else if (fabs(a[2])>1e-14 && fabs(a[0])>1e-14) {
		   if (a[2]<1e+250&&a[1]<1e+250&&a[0]<1e+250){
		                            if (fabs(a[1])<1e-14) {modd=-four*a[2]*a[0];
			     ...
			     b[1]=a[1]/(1e+250);
				 b[0]=a[0]/(1e+250);
				 if (fabs(b[1])<1e-14) {modd=-four*b[2]*b[0];
		                            ...
		                            if (fabs(modd)<1e-14) {rr[1] = (-b[1]+sqrt(fabs(b[2]))*sqrt(fabs(b[1]))*sqrt(fabs(modd)))/(two*b[2]);
