...
int rquartic_roots(double *a, double *root){
	...
	double four=4,two=2,one=1,zero=0;
	...
	if (a[3]==zero&&a[2]==zero&&a[1]==zero) {
		                                     if (a[0]==zero){root[1]=zero;root[2]=zero;root[3]=zero;root[4]=zero; return 1;}//x=0 4 times
									 ...
									 if (q1==2){
										        if (rr1[1]>=zero&&rr1[2]>=zero) {root[1]=sqrt(rr1[1]);root[2]=sqrt(rr1[2]);root[3]=-sqrt(rr1[2]);root[4]=-sqrt(rr1[2]); return 4;}//4 distinct real roots
										              ...
										              if (rr1[1]>zero) {root[1]=sqrt(rr1[1]);root[2]=sqrt(rr1[2]);root[3]=-sqrt(rr1[2]);root[4]=-sqrt(rr1[2]); return 3;}//2 pairs on distinct real roots;
													  else if (rr1[1]==zero) {root[1]=zero;root[2]=zero;root[3]=zero;root[4]=zero;return 1;}// 0 repeated 4 times
						  ...
						  if (c==0) {if (r[1]>zero) {root[1]=r[1];root[2]=zero;root[3]=r[2];root[4]=r[3]; return 2;}//two distince real and complex pair
						                  ...
						                  return 3;}
						  else if (c==3) {if (r[3]>zero) {root[1]=r[1];root[2]=r[2];root[3]=r[3];root[4]=zero;return 4;}
						                  else if (r[1]<zero) {root[1]=zero;root[2]=r[1];root[3]=r[2];root[4]=r[3];return 4;}
										  else if (r[1]==zero) {root[1]=zero;root[2]=r[1];root[3]=r[2];root[4]=r[3];return 3;}
										  else if (r[2]==zero) {root[1]=r[1];root[2]=zero;root[3]=r[2];root[4]=r[3];return 3;}
										  else if (r[3]==zero) {root[1]=r[1];root[2]=r[2];root[3]=r[3];root[4]=zero;return 3;}
	...
	else if (a[3]==zero&&a[1]==zero){a3[0]=a[0];a3[1]=a[2];a3[2]=one;		                             
									 q3=quad_roots(a,rr3);
									 if (a[0]==zero){//r1=r2=0
		              ...
		              if (fabs(rr1[1]-rr2[1])<DBL_EPSILON){root[1]=rr1[1];root[2]=rr1[2];root[3]=rr2[1];root[4]=rr2[2]; return 1;}//two paris are equal
		                  ...
		                  if (q1==1){
							        if (fabs(rr1[1]-rr2[1])<DBL_EPSILON||fabs(rr1[1]-rr2[2])<DBL_EPSILON){root[1]=rr2[1];root[2]=rr1[1];root[3]=rr1[2];root[4]=rr2[2];return 1;}
							             ...
							             if (fabs(rr1[1]-rr2[1])<DBL_EPSILON||fabs(rr1[2]-rr2[1])<DBL_EPSILON){root[1]=rr1[1];root[2]=rr2[1];root[3]=rr2[2];root[4]=rr1[2];return 1;}
		...
		if (fabs(rr1[1]-rr2[1])<DBL_EPSILON){
		                    ...
		                    return 3;}
		else if (fabs(rr1[1]-rr2[2])<DBL_EPSILON){root[1]=rr2[1];root[2]=rr1[2];root[3]=rr2[2];root[4]=rr1[2];return 3;}
		else if (fabs(rr1[2]-rr2[1])<DBL_EPSILON){root[1]=rr1[1];root[2]=rr1[2];root[3]=rr2[1];root[4]=rr2[2];return 3;}
		else if (fabs(rr1[2]-rr2[2])<DBL_EPSILON){
