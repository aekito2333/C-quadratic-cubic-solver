...
int quad_roots(double *a,double *rr){
	double r[1],b[3];
	double two=2,four=4,zero=0,three=3,modd,r1,r2,e=1e-10;
	int l;
    l=lin_root(a,r);
	if (fabs(a[2])<e) {lin_root(a,r); 
	...
	else if (fabs(a[2])>e && fabs(a[0])<e) {lin_root(a,r); 
	                               if (r[0]>zero) {rr[1]=r[0]; rr[2]=zero;}
			                       else {rr[1]=zero; rr[2]=r[0];}
	                               return 2;}//r= real and 0, ordered
	else if (fabs(a[2])>e && fabs(a[0])>e) {
		   if (a[2]<1e+308&&a[1]<1e+308&&a[0]<1e+308){
		                                if (fabs(a[1])<e) {modd=-four*a[2]*a[0];
										                 if (modd<zero){rr[1]=zero;rr[2]=sqrt(-modd)/(two*a[2]);return 0;}
														 else if (a[2]>zero) {rr[1]=sqrt(modd)/(two*a[2]);rr[2]=-sqrt(modd)/(two*a[2]);return 2;}
														 else {rr[1]=-sqrt(modd)/(two*a[2]);rr[2]=sqrt(modd)/(two*a[2]);return 2;}
														 }
		
		                            else {modd = ((a[1]/fabs(a[1]))*a[1]/fabs(a[2])-(a[2]/fabs(a[2]))*four*a[0]/fabs(a[1]));
	                                
		                            if (fabs(modd)<=1e-10) {rr[1] = (-a[1]+sqrt(fabs(a[2]))*sqrt(fabs(a[1]))*sqrt(modd))/(two*a[2]);
	                                                      rr[2] = (-a[1]-sqrt(fabs(a[2]))*sqrt(fabs(a[1]))*sqrt(modd))/(two*a[2]); 
				                                          return 1;}//repeated real roots
									else if (modd<-1e-10) {rr[1] = -a[1]/(two*a[2]);
									                     rr[2] = fabs(sqrt(fabs(a[2]))*sqrt(fabs(a[1]))*sqrt(-modd)/(two*a[2])); //+ve imaginary part
									                     return 0;}//imaginary roots 					  
														  
		                            else if (modd>1e-10) {if (a[1]<zero)
			     ...
			     b[1]=a[1]/(1e+250);
				 b[0]=a[0]/(1e+250);
				 if (fabs(b[1])<e) {modd=-four*b[2]*b[0];
									...
									if (fabs(modd)<e) {rr[1] = (-b[1]+sqrt(fabs(b[2]))*sqrt(fabs(b[1]))*sqrt(modd))/(two*b[2]);
	                                                      rr[2] = (-b[1]-sqrt(fabs(b[2]))*sqrt(fabs(b[1]))*sqrt(modd))/(two*b[2]); 
				                                          return 1;}//repeated real roots
									else if (modd<zero) {rr[1] = -b[1]/(two*b[2]);
									                rr[2] = fabs(sqrt(fabs(b[2]))*sqrt(fabs(b[1]))*sqrt(-modd)/(two*b[2])); //+ve imaginary part
									                return 0;}//imaginary roots 
