	...
	if (a[2]==zero) {lin_root(a,r); 
		   ...
		   if (a[2]<1e+308&&a[1]<1e+308&&a[0]<1e+308){
				 ...
				 if (b[1]==zero) {modd=-four*b[2]*b[0];
	                                ...
	                                if (modd<zero) {rr[1] = -b[1]/(two*b[2]);
									                rr[2] = fabs(sqrt(fabs(b[2]))*sqrt(fabs(b[1]))*sqrt(-modd)/(two*b[2])); //+ve imaginary part
									                return 0;}//imaginary roots 
		                            else if (modd==zero) {rr[1] = (-b[1]+sqrt(fabs(b[2]))*sqrt(fabs(b[1]))*sqrt(modd))/(two*b[2]);
