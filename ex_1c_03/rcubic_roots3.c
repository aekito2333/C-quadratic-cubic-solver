...
int rcubic_roots(double *a,double *r){
	...
	double three=3,two=2,twoseven=27,zero=0,one=1,nine=9,eleven=11,e=1e-10;
	int q,n;
	beta=-a[2]/three;
	...
	if (fabs(a[2])<e&&fabs(a[1])<e) {pr=cbrt(-a[0]);  
	                             r[1]=pr;r[2]=pr*(-one/two);r[3]=fabs(pr*sqrt(three)/two);
	                             return 0;}//one real two complex                            
	
	else if (fabs(a[0]-a[1]*a[2])<e) {pr=sqrt(fabs(a[1])); 
	...
	else if (fabs(a[0])>e && fabs(a[1]-one/three*a[2]*a[2]) && fabs(a[0]-one/twoseven*a[2]*a[2]*a[2])<e){
		                                            pr=-a[2]/three; r[1]=pr;r[2]=pr;r[3]=pr;return 1;}//triple root     
   
    else if (fabs(py*py*py+p*py-one)<e&&fabs(three*py*py+p)<e){p2=-three*cbrt(one/two)*cbrt(one/two);
		                             
							   if (fabs(p-p2)<e){pr1=alpha*py+beta;//repeated
		                                                   pr2=alpha*one/(py*py)+beta;
		                                                   if (pr1>pr2) {r[1]=pr1;r[2]=pr1;r[3]=pr2;}
														   else {r[1]=pr2;r[2]=pr1;r[3]=pr1;}
	                                                       return 2;}}//repeated real roots 
														   
	else if (fabs(a[0])<e||fabs(alpha)<e) {if(fabs(a[0])<e){pr=zero;
	                                              q=quad_roots(a,rr);}
			  ...
			  while ((fabs(y2-y1)>e)||fabs(y2-y1)<fabs(y1-y0)){
				  y0=y1;//y[0]
	...
	pr=py;
	     ...
	     if (q==0)      {r[1]=pr;r[2]=rr[1];r[3]=fabs(rr[2]);return 0;}//a pair of imaginary roots
		 ...
		 else if (q==2) {if (fabs(pr-rr[1])<e||fabs(pr-rr[2])<e) {if (rr[1]>rr[2]){r[1]=rr[1];r[2]=pr;r[3]=rr[2];}
						...
						if (fabs(r[1]-r[2])<e||fabs(r[1]-r[3])<e||fabs(r[2]-r[3])<e)
						{return 2;}
								else {return 3;}	                                                    
}}
}
